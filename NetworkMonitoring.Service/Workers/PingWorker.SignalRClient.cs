...
{
    public class PingWorkerSignalR : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var pl = new NetworkMonitoring.Shared.Models.PingLog { DeviceId = 1, ResultMessage = "Ping OK (from SignalR worker)", IsIssue = false, SessionId = Guid.NewGuid().ToString(), CreatedAt = DateTime.UtcNow };
                    db.PingLogs.Add(pl);
